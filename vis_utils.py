...
def class_distributions(dataset):
    ...
    autoAxis = ax.axis()
    rec = Rectangle((autoAxis[0]-0.3,autoAxis[2]),(autoAxis[1]-autoAxis[0])+0.55,(autoAxis[3]-autoAxis[2])+0.5,fill=False,lw=1,  linestyle = 'dashed')
...
def plot_files_preprocessing():
    ...
    for i in range(0, 3):
        for j in range(0, 4):
            ...
            patches, texts,autotexts = plt.pie([after_size[index], deleted[index] ], explode=expl,autopct='%1.2f%%', pctdistance=pctdist,
                      wedgeprops= {"edgecolor":"black",
                              'linewidth': 0.2,
                              'antialiased': True},textprops={'fontsize': 6},colors = ['orange', 'mediumspringgreen'])
            ...
            autoAxis = ax.axis()
            rec = Rectangle((autoAxis[0]-0.3,autoAxis[2]),(autoAxis[1]-autoAxis[0])+0.37,(autoAxis[3]-autoAxis[2])+0.512,fill=False,lw=1,  linestyle = 'dashed')
            rec = ax.add_patch(rec)
            rec.set_clip_on(False)
