from Proposed.data_utils import load_processed_dataset
...
def principal_component_varience():
    ...
    ax2 = sns.barplot(x=idx[:limit_df], y='explained variance', data=explained_variance_limited, palette='summer')
    ax2 = ax1.twinx()
    ax2.grid(False)
    ax2.set_ylabel('Cumulative', fontsize=14)
    ax2 = sns.lineplot(x=idx[:limit_df]-1, y='cumulative', data=explained_variance_limited, color='black')
    
    ax1.axhline(mean_explained_variance, ls='--', color='grey') #plot mean
    ax1.text(1.5, mean_explained_variance+(mean_explained_variance*.05), "Average", color='black', fontsize=14) #label y axis
