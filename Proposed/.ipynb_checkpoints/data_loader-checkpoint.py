...
class CIC2019(Dataset):
    ...
    def __init__(self, kind='train', n_features=10):
        if kind=='train':
            ...
            xy = pd.read_csv(os.getcwd()+'/Datasets/train_PCA_Dataset.csv', nrows = 5000)
            xy[' Label'] = xy[' Label'].apply(map_binary_class_attack)
            
            self.labels = torch.Tensor(xy[' Label'])
            xy.drop(['Unnamed: 0', ' Label'], axis=1, inplace=True)
            print("******************** Train PCA Dataset Loaded **********************")
    
        else:
            print("******************** Loaing Test PCA Dataset **********************")

            xy = pd.read_csv(os.getcwd()+'/Datasets/test_PCA_Dataset.csv', nrows=5000)
            xy[' Label'] = xy[' Label'].apply(map_binary_class_attack)
...
class CIC2019Multi(Dataset):
    ...
    def __init__(self, kind='train', n_features=10):
        if kind=='train':
            print("******************** Loaing Train PCA Dataset **********************")
            xy = pd.read_csv(os.getcwd()+'/Datasets/train_PCA_Dataset.csv', nrows = 50000)    
            ...
            xy = pd.read_csv(os.getcwd()+'/Datasets/test_PCA_Dataset.csv', nrows=50000)
            xy[' Label'] = xy[' Label'].apply(map_multi_class_attack)
